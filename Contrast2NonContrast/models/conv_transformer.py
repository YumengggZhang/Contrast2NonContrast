    ...
    def __init__(self, output_nc, ngf=16, norm_layer=nn.BatchNorm2d, n_downsampling=3):
            ...
            for i in range(n_downsampling):  # add upsampling layers
                ...
                model += [nn.Upsample(scale_factor=2, mode='nearest'),
                        nn.Conv2d(int(ngf * mult), int(ngf * mult / 2), kernel_size=3, stride=1, padding=1, bias=use_bias),
                        norm_layer(int(ngf * mult / 2)),
                        nn.ReLU(True)]
...
class ConvTransformer(nn.Module):
    def __init__(self, input_nc, n_downsampling, depth, heads, proj_kernel=3,
        ...
        self.conv_decoder = DecoderNew(output_nc=input_nc, ngf=ngf, n_downsampling=n_downsampling)
